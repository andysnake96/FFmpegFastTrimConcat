...
def GetGroups(srcItems=None, grouppingRule=ffmpegConcatDemuxerGroupping, startSearch=".", filterTumbNailPresent=False):
    ...
    @param srcItems:    vid items list to group as dict itemNameID->item,
                        if None will be generated by a recoursive search from @param startSearch
    @param grouppingRule: function: vidItem->label (label= immutable key for groups dict) -> see grouppingFunction.py
    @param filterTumbNailPresent: filter away vids without a tumbnail img file
    ...
    for item in FilterItems(srcItems, tumbNailPresent=filterTumbNailPresent):
